...
- `app/`, `components/`, and the rest of the Next.js codebase are frontend-only.
- `backend/` contains the Spring Boot + Maven backend.
...
If you would rather proxy backend calls through the Next.js app, you can use `BACKEND_BASE_URL=http://localhost:8080` instead. When no backend URL is configured in production, the frontend now falls back to same-origin `/api` requests.

**Backend Environment Variables**

The Java backend reads its configuration from `backend/.env`.

At minimum, make sure these values are set there:
...
`FRONTEND_BASE_URL` is used for links generated by the backend, such as email CTAs. CORS is controlled separately through `CORS_ALLOWED_ORIGIN_PATTERNS`, which now defaults to `*` so a deployed frontend can connect before you know its final URL.
...
1. Start the Spring Boot backend from `backend/`.
2. Start the Next.js frontend from the project root.
3. Open [http://localhost:3000](http://localhost:3000) in your browser.

## <a name="render-deploy">🚢 Render Deployment</a>

Use the repo root as the Render service root. Render will detect the root `Dockerfile`, build the Spring Boot backend, and start it with the container `ENTRYPOINT`, so you do not need custom build or start commands.
...
- `CORS_ALLOWED_ORIGIN_PATTERNS=*`
- Any optional SMTP or scheduler values you need

For the frontend deployment, set one of these:

- `BACKEND_BASE_URL=https://your-backend-service.onrender.com` to proxy `/api/*` requests through Next.js
- `NEXT_PUBLIC_BACKEND_BASE_URL=https://your-backend-service.onrender.com` to call the backend directly from the browser
